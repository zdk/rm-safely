...
create_hook_file() {
    cat >"$HOOK_FILE" <<'EOF'
#!/bin/bash
# rm-hook - rm alias that backs up files to a trash directory before deletion
                ...
                if cp -rp "$arg" "$TRASH_DIR/$trash_name" 2>/dev/null; then
                    if [ -e "$TRASH_DIR/$trash_name" ]; then
                        ...
                        if [ -d "$arg" ]; then
                            ...
                            echo "Backed up '$arg' to $TRASH_DIR/$trash_name"
                        fi
                    else
                    ...
                    if [ ! -r "$arg" ]; then
                        ...
                        available_space=$(df "$TRASH_DIR" 2>/dev/null | awk 'NR==2 {print $4}')
                        ...
                        if [ -n "$file_size" ] && [ -n "$available_space" ] && [ "$file_size" -gt "$available_space" ]; then
                            ...
                            echo "ERROR: Failed to backup '$arg' to %TRASH_DIR"
                        fi
                    fi
                    failed_list="$failed_list $arg"
                fi
        ...
        if [ -n "$backup_list" ]; then
            ...
            for backup_entry in $backup_list; do
                if [ -n "$backup_entry" ]; then
                    ...
                    if [ -e "$TRASH_DIR/$trash_name" ]; then
                        /bin/rm -rf "$TRASH_DIR/$trash_name"
                    fi
                fi
            done
            unset IFS
